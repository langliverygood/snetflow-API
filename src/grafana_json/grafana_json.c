...
static char* grafana_build_reponse_query_top(MYSQL* mysql, const char* request_body, grafana_query_request_s* rst, snetflow_job_s* job)
{
    ...
    ret = get_top(mysql, job->start_time, job->end_time, cfg, &top_map);
    if (ret != 0) {
        return NULL;
    }
    /* 根据target拼接json */
...
static char* grafana_build_reponse_query_trend(MYSQL* mysql, const char* request_body, grafana_query_request_s* rst, snetflow_job_s* job)
{
    ...
    ret = get_trend(mysql, job->start_time, job->end_time, cfg, &trend_map);
    if (ret != 0) {
        return NULL;
    }
    /* 根据target拼接json */
...
static char* grafana_build_reponse_query_history(MYSQL* mysql, const char* request_body, grafana_query_request_s* rst, snetflow_job_s* job)
{
    ...
    ret = get_history(mysql, job->start_time, job->end_time, cfg, &history_vec);
    if (ret != 0) {
        return NULL;
    }
    /* 根据target拼接json */
...
static char* grafana_build_reponse_query_warning(MYSQL* mysql, const char* request_body, grafana_query_request_s* rst, snetflow_job_s* job)
{
    ...
    ret = get_warning(mysql, job->start_time, job->end_time, cfg, &warning_map);
    if (ret != 0) {
        return NULL;
    }
    /* 根据target拼接json */
...
static char* grafana_build_reponse_query_associate(MYSQL* mysql, const char* request_body, grafana_query_request_s* rst, snetflow_job_s* job)
{
    ...
    ret = get_associate(mysql, job->start_time, job->end_time, cfg, &associate_map, ip);
    if (ret != 0) {
        return NULL;
    }
    /* 根据target拼接json */
